...
class LINESingleDatasetBase(ObjectDetectionDatasetBase):
    def __init__(self, coco_dir, focus, ignore=None, transform=None, target_transform=None, augmentation=None, class_labels=None):
        ...
        self._annopath = os.path.join(self._coco_dir, focus, 'scene_gt_coco_modal.json')
        if os.path.exists(self._annopath):
            self._coco = COCO(self._annopath)
        else:
            raise FileNotFoundError('json: {} was not found'.format('scene_gt_coco_modal.json'))


        # remove no annotation image
        self._imageids = list(self._coco.imgToAnns.keys())  #画像のIDを取得、リストにまとめる 0~999
...
class LINEMultiDatasetBase(Compose):
    def __init__(self, **kwargs):
        ...
        if isinstance(coco_dir, str) and isinstance(focus, str):
            ...
            datasets = [LINESingleDatasetBase(cdir, f, **kwargs) for cdir, f in zip(coco_dir, focus)]
